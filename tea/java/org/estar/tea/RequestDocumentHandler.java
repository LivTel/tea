// $Header: /space/home/eng/cjm/cvs/tea/java/org/estar/tea/RequestDocumentHandler.java,v 1.24 2016-05-23 14:29:12 cjm Exp $
...
public class RequestDocumentHandler implements Logging {
	...
	public static final double DEFAULT_SEEING_CONSTRAINT = 2.0;
	 ...
	 *            The eSTARIO.
	 * @param handle
	 *            Globus IO Handle for the connection.
	 */
	...
	/**
	 * Called to handle an incoming observation request document. Attempts to
	 * add a group to the OSS Phase2 DB.
	 * 
	 * @param document
	 *            The RTML request document.
	 * @throws Exception
	...
	public RTMLDocument handleRequest(RTMLDocument document) throws Exception {
		...
		while (iobs.hasNext()) {
			...
			addsource.setClientDescriptor(new ClientDescriptor("EmbeddedAgent", ClientDescriptor.ADMIN_CLIENT,
					ClientDescriptor.ADMIN_PRIORITY));
			addsource.setCrypto(new Crypto("TEA"));

			addsource.setTransactionPriority(0);

			JMSCommandHandler client = new JMSCommandHandler(tea.getConnectionFactory(), addsource,
					tea.getOssConnectionSecure());
			client.send();

			if (client.isError()) {
				logger.log(INFO, 1, CLASS, cid, "executeRequest", "Reply was: " + client.getReply());
				if (client.getReply() != null && client.getReply().getErrorNum() == ADD_SOURCE.SOURCE_ALREADY_DEFINED) {
			...
			addcfg.setClientDescriptor(new ClientDescriptor("EmbeddedAgent", ClientDescriptor.ADMIN_CLIENT,
					ClientDescriptor.ADMIN_PRIORITY));
			addcfg.setCrypto(new Crypto("TEA"));

			addcfg.setTransactionPriority(0);

			JMSCommandHandler client2 = new JMSCommandHandler(tea.getConnectionFactory(), addcfg,
					tea.getOssConnectionSecure());
		...
		addgroup.setClientDescriptor(new ClientDescriptor("EmbeddedAgent", ClientDescriptor.ADMIN_CLIENT,
				ClientDescriptor.ADMIN_PRIORITY));
		...
		JMSCommandHandler client = new JMSCommandHandler(tea.getConnectionFactory(), addgroup,
				tea.getOssConnectionSecure());
		...
		if (client.isError()) {
			logger.log(INFO, 1, CLASS, cid, "handleRequest",
					"Internal error during ADD_GROUP: " + client.getErrorMessage());
		...
		// observation needs to be declared global.- look at UH which defines on
		// ObsInfo.
		...
		// Initialize and start the ARQ as UpdateHandler. If the ARQ does not
		// successfully
...
// Revision 1.23 2008/09/15 12:10:38 cjm
// Added extra logging.
//
...
// RTML setType calls replaced by equivalent RTMLDocument methods for version
// independant values.
